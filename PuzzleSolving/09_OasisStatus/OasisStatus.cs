...
public static class OasisStatus
{
    ...
    private static long PredictNextStatus(List<long> currentSequence, long firstValue, long currentOffset = 0L)
    {
        if (currentSequence.All(entry => entry.Equals(currentSequence.First())))
        {
